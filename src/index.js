...
///차트3 : 시간대별 요일별 이용 현황 히트맵///
        ...
        var heatmapChart = function() {
          d3.csv("./src/data/top10_요일별_시간대별_이용건수_heatmap.csv", function(data) {
            data2 = data.filter(p => p.place ==global_name); //클릭된 대여소의 정보가 나타나도록 처리
...
///차트4 : 대여소 Hotspot 순위별로 보여주는 버블차트 ///
      ...
      "translate(" + 0+ "," + 10 + ")");
        ...
        //데이터 읽기
        d3.csv("./src/data/rank.csv", function(data) {
        ...
        var myColor =["#e8f6f9","#d5efed","#b7e4da","#8fd3c1","#68c2a3","#49b17f","#2f9959","#157f3c","#036429","#00441b"]; // 색상 정의 
        ...
        .attr("cx", function (d) { if(d.value == 1) {return x(0.6);} else {return x(0.1+ d.rank * 0.9);} } )
        ...
        circles.on("click", function(n){ // 버블 클릭시, 발생 동작 정의
          ...
          global_name = n.name; // global_name을 클릭된 버블에 대한 대여소이름으로 변경
...
legend_data = ['Hotspot', 'Hotspot Top10 대여소','Hotspot <-> 대여소'];
